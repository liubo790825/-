...
namespace LeaRun.Application.AppSerivce
{
    ...
    public class LoginModule : BaseModule
    {
        ...
        private OrganizeBLL organizeCache = new OrganizeBLL();
        private DepartmentBLL departmentCache = new DepartmentBLL();
        private RoleBLL roleCache = new RoleBLL();
            ...
            : base("/learun/api")
        {
            ...
            Post["/login/stuLogin"] = StuLogin;//学生登录
        }
        ...
        //学生登录
        private Negotiator StuLogin(dynamic _)
        {
            var recdata = this.GetModule<ReceiveModule<stuData>>();
            LogEntity logEntity = new LogEntity();
            logEntity.CategoryId = 1;
            logEntity.OperateTypeId = ((int)OperationType.Login).ToString();
            logEntity.OperateType = EnumAttribute.GetDescription(OperationType.Login);
            logEntity.OperateAccount = recdata.data.username;
            logEntity.OperateUserId = recdata.data.userid;
            logEntity.Module = "敏捷开发框架";

            BK_StuInfoBLL bll = new BK_StuInfoBLL();
            BK_StuInfoEntity userEntity = new BK_StuInfoEntity();
            try
            {
                stuData result = new stuData();
                Operator operators = new Operator();
                #region 第三方账户验证
                //AccountEntity accountEntity = accountBLL.CheckLogin(recdata.data.username, recdata.data.password);
                //if (accountEntity != null)
                ...
                //    LoginLimit(recdata.data.username, operators.IPAddress, operators.IPAddressName, recdata.platform);
                //    result.userid = operators.UserId;
                //    result.username = operators.UserName;
                //    result.password = operators.Password;
                //}
                #endregion
                #region 内部登录
                ...
                {
                    //写入当前用户信息
                    userEntity = bll.StuLogin(recdata.data.username, recdata.data.password);
                    //UserEntity userEntity = new UserBLL().CheckLogin(recdata.data.username, recdata.data.password);
                    if (userEntity != null)
                    {
                        //AuthorizeBLL authorizeBLL = new AuthorizeBLL();
                        operators.UserId = userEntity.IdentityCardNo;
                        //operators.Code = userEntity.EnCode;
                        //operators.Account = userEntity.Account;
                        operators.UserName = userEntity.IdentityCardNo;
                        operators.Password = userEntity.StuPwd;
                        //operators.Secretkey = userEntity.Secretkey;
                        //operators.CompanyId = userEntity.OrganizeId;
                        //operators.DepartmentId = userEntity.DepartmentId;
                        operators.IPAddress = Net.Ip;
                        ...
                        result.userid = operators.UserName;
                        result.username = operators.UserName;
                        result.password = operators.Password;
                    }
                    else
                    {
                        return this.SendData(ResponseType.Fail, "无此用户，请重试");
                    }
                }
                #endregion
                //移动端不采用cookie的方式
                ...
                //移动端不采用cookie的方式
                //this.WriteCache<BK_StuInfoEntity>(stuEntity, stuEntity.IdentityCardNo);
                ...
                return this.SendData<stuData>(result, result.userid, operators.Token, ResponseType.Success);
                //return this.SendData<BK_StuInfoEntity>(userEntity, userEntity.IdentityCardNo, operators.Token, ResponseType.Success);
            }
            catch (Exception ex)
            {
                logEntity.ExecuteResult = -1;
    ...
    public class stuData
    {
        ...
        /// 存一个stuuser对象
        /// </summary>
        public BK_StuInfoEntity entity { get; set; }
    }
