...
namespace LeaRun.Util.FlowWork
{
    ...
    /// 描 述：工作流流程流转操作类
    /// </summary>
    public class WRuntime : IWRuntime
    {
        private WRuntimeModel _runtimeModel = null;
        ...
        public WRuntime(WRuntimeInitModel wfRuntimeInitModel,GetFrmData getFrmData = null)
        {
            _runtimeModel = new WRuntimeModel();
            ...
            {
                _runtimeModel.nextNodeId = GetNextNode(wfRuntimeInitModel.frmData);//.frmData);//下一个节点
            ...
            ///_runtimeModel.sqlFrm = SqlBuider(schemeContentJson, wfRuntimeInitModel.frmData, wfRuntimeInitModel.processId);//去掉这个执行的SQL语名
        }
        ...
        private Dictionary<string, dynamic> GetNodeDictionary(dynamic schemeContentJson)
        {
            ...
            if (schemeContentJson.Flow!=null)
            {
            ...
            if (schemeContentJson.nodes!=null)
            {
                foreach (var item in schemeContentJson.nodes)
                {
                    if (!nodeDictionary.ContainsKey(item.id.Value))
                    {
                        nodeDictionary.Add(item.id.Value, item);
        ...
        private Dictionary<string, List<dynamic>> GetLineDictionary(dynamic schemeContentJson)
        {
            ...
            if (schemeContentJson.lines!=null)
            {
                foreach (var item in schemeContentJson.lines)
                {
                    if (!lineDictionary.ContainsKey(item.from.Value))
                    {
                        List<dynamic> d = new List<dynamic>();
        ...
        private string GetNextNode(string frmData, string nodeId = null)
        {
            try
            {
                ...
                else if (frmData != "")
                {
                    ...
                    dynamic frmDataJson = frmData.ToJson();//.ToJObject();//获取数据内容
                    ...
                    foreach (var item in LineList)//轮训该节点所有连接的线路
                    {
                        ...
                        foreach (var _item in item.setInfo.conditionJson)//.ConditionValueJson)//轮询该线条上的所有条件
                        {
                            var _compareField = _item.FieldId.Value.Split('|')[1].ToLower();
                            string frmvalue = "";//得到值
                            foreach (var itemFrmData in frmDataJson)
                            {
                                if (itemFrmData.field.Value.Equals(_compareField))
                                {
                                    frmvalue = itemFrmData.realvalue.Value;
                                    break;
                                }
                            }
                            if (!string.IsNullOrEmpty(frmvalue))
                            {
                                string operation = _item.FilterId.Value;//得到比较的方式值，LessThan,NotEqual,Greater
                                string paramValue = _item.FilterValue.Value;//得到比较的值，
                                bool compareValue = LineCompared(frmvalue, operation, paramValue);

                                if (_item.Logic.Value == "AND")
                                {
                                    flag = compareValue;
                                    if (!compareValue)
                                    {
        ...
        /// </summary>
        /// <param name="frmData">表单数据（用于判断流转条件）</param>
        private string GetNextNode_old(string frmData,string nodeId = null)
        {
            try
            {
                List<dynamic> LineList = null;
                ...
                {
                    frmData = frmData.ToLower();//统一转小写
                    dynamic frmDataJson = frmData.ToJObject();//获取数据内容
                    ...
                    foreach (var item in LineList)//轮训该节点所有连接的线路
                    {
                        ...
                        foreach (var _item in item.setInfo.conditionJson)//.ConditionValueJson)//轮询该线条上的所有条件
                        {
                            ...
                            {
                                string frmvalue = frmDataJson[_item.ParamName.Value.ToLower()].ToString();//得到值
                                string operation = _item.Operation.Value;//得到比较的方式值，如AND
                                string paramValue = _item.ParamValue.Value;//得到比较的值，
                                bool compareValue = LineCompared(frmvalue, operation, paramValue);
        ...
        /// </summary>
        /// <returns></returns>
        public WRuntimeModel runtimeModel
        {
