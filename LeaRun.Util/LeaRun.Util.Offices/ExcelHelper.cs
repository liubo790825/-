...
namespace LeaRun.Util.Offices
{
    /// <summary>
    ///版 本 V1.0
    ///Copyright (c) 2010-2015 北京泉江科技有限公司　　　　　　　　　　　　　　　　　　　　　　　　　　
    ///创建人：L&J
    ///日 期：2015/11/25
    ...
    public class ExcelHelper
    {
        ...
        public static void ExcelDownload(ExcelConfig excelConfig)
        {
            HttpContext curContext = HttpContext.Current;
            // 设置编码和附件格式
            curContext.Response.ContentType = "application/ms-excel";
            ...
            curContext.Response.AppendHeader("Content-Disposition",
                "attachment;filename=" + HttpUtility.UrlEncode(excelConfig.FileName, Encoding.UTF8));
            //调用导出具体方法Export()
            curContext.Response.BinaryWrite(ExportMemoryStream(excelConfig).GetBuffer());
            curContext.Response.End();
        }
        ...
        public static MemoryStream ExportMemoryStream(DataTable dtSource, ExcelConfig excelConfig)
        {
            ...
            for (int i = 0; i < dtSource.Columns.Count; )
            {
                ...
                //bool flag = true;
                if (i < excelConfig.ColumnEntity.Count)// 这里有可能会出错，dtSource.Columns可能会比excelConfig.ColumnEntity.Count大，造成excelConfig.ColumnEntity[colint].Column超出
                {
            ...
            #region 右击文件 属性信息
            {
                ...
                si.Author = "L&J"; //填加xls文件作者信息
                si.ApplicationName = "泉江信息"; //填加xls文件创建程序信息
                si.LastAuthor = "L&J"; //填加xls文件最后保存者信息
                si.Comments = "L&J"; //填加xls文件作者信息
        ...
        public static MemoryStream ExportMemoryStream(ExcelConfig excelConfig)
        {
            ...
            #region 右击文件 属性信息
            {
                ...
                SummaryInformation si = PropertySetFactory.CreateSummaryInformation();
                si.Author = "tercel"; //填加xls文件作者信息
                si.ApplicationName = "翔飞信息"; //填加xls文件创建程序信息
                si.LastAuthor = "tercel"; //填加xls文件最后保存者信息
                si.Comments = "tercel"; //填加xls文件作者信息
                si.Title = "标题信息"; //填加xls文件标题信息
                si.Subject = "主题信息";//填加文件主题信息
                si.CreateDateTime = System.DateTime.Now;
                workbook.SummaryInformation = si;
            ...
            #region 如果设置了列标题就按列标题定义列头，没定义直接按字段名输出
            int i = 0;
            foreach (ColumnEntity item in excelConfig.ColumnEntity)
            {
                headerRow.CreateCell(i).SetCellValue(item.ExcelColumn);
                headerRow.GetCell(i).CellStyle = cHeadStyle;
                //设置列宽
                sheet.SetColumnWidth(i, (Encoding.GetEncoding(936).GetBytes(item.ExcelColumn).Length + 1) * 256);
                i++;
            }
            #endregion
            using (MemoryStream ms = new MemoryStream())
