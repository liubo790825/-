    ...
    public class WFProcessInstanceService : RepositoryFactory, WFProcessInstanceIService
    {
        ...
        public DataTable GetToMeBeforePageList(Pagination pagination, string queryJson)
        {
            try
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT
                                    ROW_NUMBER() OVER(order by w.CreateDate desc) as rownum,
	                                w.Id as f_id,
                                    w.Code as f_code,
									w.CustomName as f_name,	                                
	                                w.ProcessSchemeId as f_processschemeid,
                                    w.SchemeType as f_schemetype,
                                    t2.ItemName AS f_schemetypename,                                   
                                    w.EnabledMark as f_enabledmark,
	                                w.CreateDate as f_createdate,	                               
	                                w.CreateUserName as f_createusername,                                   
                                    w.Description as f_description,
                                    w.wfLevel as f_wfevel,
									t3.F_Id as nodeid,
									t3.F_NodeId,
									t3.F_NodeName,
									t3.F_NodeSate,
        ...
        /// <param name="queryJson"></param>
        /// <returns></returns>
        public DataTable GetToMeBeforePageList_old(Pagination pagination, string queryJson)
        {
            try
            {
                var strSql = new StringBuilder();
