(function (n) {
    ...
    t = {
        init: function () {
            ...
            o = top.learun.request("btnId");
            var exid = top.learun.data.get(["excelImportTemplate", e]);
            $.SetForm({
                url: "../../SystemManage/ExcelImport/GetFormJsonByModuleId",
                param: {moduleId:e,btnId:o},
                success: function (data) {
                    r = data;
                }
            });
            if (!r) return top.learun.dialogTop({
                msg: "获取不到模板数据",
                type: "error"
            }),
            /*
            if (r = top.learun.data.get(["excelImportTemplate", e, "entitys", o, "data"]), console.log(r), !r) return top.learun.dialogTop({
                msg: "获取不到模板数据",
                type: "error"
            }),*/
            !1;
        ...
        initButton: function (i) {
            n("#lr-upfile").uploadifyEx({
                url: "/Utility/ExecuteImportExcel?templateId=" + i.templateInfo.F_ExcelImportTemplateId,
                btnName: "上传文件",
                type: "uploadify",
                height: 31,
                width: 90,
            ...
            function () {
                top.learun.downFile({
                    ...
                    data: "templateId=" + i.templateInfo.F_ExcelImportTemplateId,
                    method: "post"
                })
            })
        },
        ...
        loadData: function (r) {
            ...
            console.log(r);
            try {
                top.learun.currentIframe().$("#gridTable").trigger("reloadGrid")
            } catch (h) { }
