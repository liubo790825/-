...
function (n) {
    n.fn.flowDesigner = function (t) {
        function e() {
            var f = {},
            e, r, u;
            for (e in t.frmData) r = t.frmData[e],
            f[r.control_field] = r.control_label;
            u = {};
            n.ajax({
            ...
            function (n, t) {
                var r, e, c, v, n, l, a;
                if (console.log(t), t.type != "startround" && t.type != "endround" && t.setInfo != undefined) {
                    if (r = "", r += '<div class="flow-portal-panel-title"><i class="fa fa-navicon"><\/i>&nbsp;&nbsp;基本信息<\/div>', r += "<ul>", r += "<li>节点标识:" + t.setInfo.nodeCode + "<\/li>", r += "<li>驳回类型:" + o[t.setInfo.nodeRejectType] + "<\/li>", r += "<li>执行对象:" + t.setInfo.nodeSysFn + "<\/li>", t.setInfo.nodeDes != "" && (r += "<li>备注:" + t.setInfo.nodeDes + "<\/li>"), t.setInfo.nodeConfluenceType != undefined && t.setInfo.nodeConfluenceType != "" && (r += "<li>会签策略:" + h[t.setInfo.nodeConfluenceType] + "<\/li>", t.setInfo.nodeConfluenceType == 2 && (r += "<li>会签比例:" + t.setInfo.nodeConfluenceRate + "<\/li>")), r += "<\/ul>", r += '<div class="flow-portal-panel-title"><i class="fa fa-navicon"><\/i>&nbsp;&nbsp;审核者<\/div>', r += "<ul>", r += "<li>类型:" + s[t.setInfo.nodeDesignate] + "<\/li>", t.setInfo.nodeDesignateData != undefined) {
                        e = "";
                        for (n in t.setInfo.nodeDesignateData.role) c = t.setInfo.nodeDesignateData.role[n],
                        e += ' <span class="label label-success">' + top.learun.data.get(["role", c, "FullName"]) + "<\/span>",
                        ...
                        e += ' <span class="label label-info">' + top.learun.data.get(["post", c, "FullName"]) + "<\/span>",
                        ...
                        e += ' <span class="label label-warning">' + top.learun.data.get(["userGroup", c, "FullName"]) + "<\/span>",
                        ...
                        e += ' <span class="label label-danger">' + top.learun.data.get(["user", c, "FullName"]) + "<\/span>",
        ...
        function (n, t) {
            if (t.F_NodeType == "startround") i.find("#" + t.F_NodeId).css("background", "#5cb85c");
            else {
                t.F_IsActivtyId == 1 && i.find("#" + t.F_NodeId).css("background", "#5bc0de");
                switch (t.F_NodeSate) {
                    case 2:
                        i.find("#" + t.F_NodeId).css("background", "#d9534f");
                        break;
                    case 1:
                        i.find("#" + t.F_NodeId).css("background", "#5cb85c");
                        break;
                    case 3:
                        i.find("#" + t.F_NodeId).css("background", "#f0ad4e")
                }
            }
            if (t.F_NodeSate != 0 && t.F_NodeSate != 10) {
                var r = '<div style="text-align:left">';
                r += "<p>处理人：" + top.learun.data.get(["user", t.F_ModifyUserId, "RealName"]); + "</p>";
                r += "<p>结果：" + {
                    "1": "通过",
                    "2": "不通过"
                }[t.F_NodeSate] + "<\/p>";
                r += "<p>处理时间：" + t.F_ModifyDate + "<\/p>";
                r += "<p>处理意见：" + t.F_Description + "<\/p><\/div>";
                i.find("#" + t.F_NodeId).attr("data-toggle", "tooltip");
                i.find("#" + t.F_NodeId).attr("data-placement", "bottom");
                i.find("#" + t.F_NodeId).attr("title", r)
            }
