...
.controller('lrAppCtrl', ['$rootScope', '$scope', '$lrGetIframe', '$http', '$lrdialogTop', function ($rootScope, $scope, $lrGetIframe, $http, $lrdialogTop) {
    ...
    $scope.btnDownProject = $rootScope.designer.downProject;
    //$scope.isTabsAdded = true;
    //加载完成开始初始化
    $rootScope.$on('iframeLoaded', function (event) {
        if ($rootScope.designer.currentPage.id == undefined) {
            $http.get($rootScope.designer.api.getProjectUrl + "?keyValue=" + $rootScope.designer.projectId)
               .success(function (data) {
                   $rootScope.designer.vm.isBeginView = true;
                   var tempData = $rootScope.designer.templates.initTemplates(data.templates);
                   $rootScope.designer.templates.fnPageTree.refresh();
                   $rootScope.designer.templates.fnTabsTree.refresh();
                   $rootScope.designer.projectName = data.project.F_Name;
                   $rootScope.designer.projectIcon = data.project.F_Icon;
...
.factory('$lrmTemplates', ['$rootScope', '$lrfnGuid', '$lrdialogTop', function ($rootScope, $lrfnGuid, $lrdialogTop) {//模板类型(树形菜单)
    ...
    var doPageButton = function (data) {
    ...
    return {
        ...
        initTemplates: function (data) {
            ...
            for (var i in pageTemplates) {
                pageTemps[pageTemplates[i].value] = pageTemplates[i];
            }

            var pageIndex = {}, pageNum = 0;
            for (var i in data) {
                var item = data[i];
                if (item.F_Type == "Page") {
                    if (item.F_Value == "lrTabs") {
                        hasTabsTemplates[0].id = item.F_Id;
                        hasTabsTemplates[0].text = item.F_Name;
                        hasTabsTemplates[0].value = item.F_Value;
                        hasTabsTemplates[0].img = item.F_img;
                        hasTabsTemplates[0].parentnodes = item.F_Parent;
                        hasTabsTemplates[0].hasChildren = true;
                        hasTabsTemplates[0].isexpand = true;
                        hasTabsTemplates[0].attr = JSON.parse(item.F_Content);
                    }
                    else {
                        var point = {
                            id: item.F_Id,
                            ...
                            value: item.F_Value,
                            img: item.F_img,
                            parentnodes: item.F_Parent,
                            hasChildren: false,
                            isexpand: false,
                            attr: JSON.parse(item.F_Content),
                            setValue: setBlankPageValue,
                            doButton: doPageButton
                        };
                        hasTemplates.push(point);
                        pageIndex[item.F_Id] = pageNum;
                        pageNum++;
                    }
                }
                else if (item.F_Type == "Component") {
                    if (item.F_Value == "lrTab") {
                        var point = {
                            id: item.F_Id,
                            text: item.F_Name,
                            value: item.F_Value,
                        ...
                        if (hasTabsTemplates[0].ChildNodes == undefined) {
                            hasTabsTemplates[0].ChildNodes = [];
                        }
                        hasTabsTemplates[0].ChildNodes.push(point);
                    }
                    else {
                        //console.log($rootScope.designer.components.getOne(item.F_Value), item.F_Value);
                        var point = {
                            id: item.F_Id,
                            text: item.F_Name,
                            value: item.F_Value,
                            ...
                            isexpand: false,
                            attr: JSON.parse(item.F_Content),
                            setValue: $rootScope.designer.components.getOne(item.F_Value).setValue,
                            doButton: $rootScope.designer.components.doRemove
                        };
                        hasTemplates[pageIndex[item.F_Parent]].hasChildren = true;
                        hasTemplates[pageIndex[item.F_Parent]].isexpand = true;
                        if (hasTemplates[pageIndex[item.F_Parent]].ChildNodes == undefined) {
                            hasTemplates[pageIndex[item.F_Parent]].ChildNodes = [];
                        }
                        hasTemplates[pageIndex[item.F_Parent]].ChildNodes.push(point);
                    }
                }
            }
        },
        initTabs: function (data) {
            ...
            if (data == undefined) {
                hasTabsTemplates[0].id = $lrfnGuid();
                var _data = [
                {
                    id: $lrfnGuid(),
                    text: "首页",
                    ...
                    hasChildren: false,
                    isexpand: false,
                    parentnodes: hasTabsTemplates[0].id,
                    value: "lrTab",
                    attr: {
                        value: "lrTab",
                        iconOn: "ion-ios-home",
...
.factory('$lrmDesigner', ['$rootScope', '$lrmComponents', '$lrmTemplates', '$http', '$lrdialogTop', '$lrGetIframe', '$lrfnGuid', function ($rootScope, $lrmComponents, $lrmTemplates, $http, $lrdialogTop, $lrGetIframe, $lrfnGuid) {
    var designer = {
        ...
        api: {
            ...
            downProjectUrl: "../../AppManage/AppProjects/DownForm",
            downProjectFileUrl: "../../AppManage/AppProjects/DownFile"
        },
        ...
        saveProject: function () {
            var _projectEntity = getPostData();
            if (_projectEntity == false) {
                return false;
            }
        ...
        downProject: function () {
            ...
            $http.post(designer.api.downProjectUrl + "?keyValue=" + designer.projectId, _projectEntity)
            .success(function (data) {
                //console.log(data);
                $.downFile(designer.api.downProjectFileUrl + "?filename=" + data.path);
                $lrdialogTop('下载成功', 'success');
            })
            .error(function () {
                $lrdialogTop('下载失败', 'error');
            });
        }
    };
    function getPostData() {
        ...
        var _projectEntity = {
            ...
            "F_IsTabed": designer.isTabsAdded ? 1 : 0
        };
        ...
        //需要上传的模板
        var _Templates = [];
        var _AllTemplates = designer.templates.getHasTemps();

        var _TabsTemplates = designer.templates.getHasTabsTemps();
        for (var i in _AllTemplates) {
            var _page = _AllTemplates[i];

            var _point = {
                "F_Id": _page.id,
                "F_Name": _page.text,
                "F_Value": _page.value,
                "F_Content": JSON.stringify(_page.attr),
                "F_Type": "Page",
                "F_Parent": "0",
                "F_level": 0,
                "F_img": _page.img
            };
            _Templates.push(_point);
            if (_page.ChildNodes != undefined && _page.ChildNodes.length > 0) {
                for (var j in _page.ChildNodes) {
                    var _component = _page.ChildNodes[j];
                    var _point = {
                        "F_Id": _component.id,
                        "F_Name": _component.text,
                        "F_Value": _component.value,
                        "F_Content": JSON.stringify(_component.attr),
                        "F_Type": "Component",
                        "F_Parent": _component.parentnodes,
                        "F_level": 1,
                        "F_img": _component.img
                    };
                    _Templates.push(_point);
                }
            }
        }
        for (var i in _TabsTemplates) {
            var _tabs = _TabsTemplates[i];
            var _point = {
                "F_Id": _tabs.id,
                "F_Name": _tabs.text,
                "F_Value": _tabs.value,
                "F_Content": JSON.stringify(_tabs.attr),
                "F_Type": "Page",
                "F_Parent": "0",
                "F_level": 0,
                "F_img": _tabs.img
            ...
            if (_tabs.ChildNodes != undefined && _tabs.ChildNodes.length > 0) {
                for (var j in _tabs.ChildNodes) {
                    var _tab = _tabs.ChildNodes[j];
                    var _point = {
                        "F_Id": _tab.id,
                        "F_Name": _tab.text,
                        "F_Value": _tab.value,
                        "F_Content": JSON.stringify(_tab.attr),
                        "F_Type": "Component",
                        "F_Parent": _tab.parentnodes,
                        "F_level": 1,
                        "F_img": _tab.img
                    };
                    _Templates.push(_point);
        ...
        _projectEntity.F_Templates = _Templates;
