...
namespace LeaRun.Application.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataBaseTableController : MvcControllerBase
    {
        ...
        public ActionResult GetTableFiledTreeJson(string dataBaseLinkId, string tableName, string nameId)
        {
            ...
            {
                item.remark = item.remark.Replace("（", "").Replace("）", "").Replace("(", "").Replace(")", "").Replace(",", "").Replace(".", "").Replace("-", "").Replace("，", "").Replace("\r", "").Replace("\n", "");
                tree = new TreeEntity();
                tree.id = item.column;
                tree.text = item.remark + "（" + item.column + "）";
                tree.value = item.remark;                
                tree.parentId = tableName;
                tree.Attribute = "datatype";//字段类型，以前是没有的
                tree.AttributeValue = GetFieldTypeToCN(item.datatype);
        ...
        /// <returns></returns>
        private string GetFieldTypeToCN(string field)
        {
            string fieldcn = "";
            if (!string.IsNullOrEmpty(field))
            {
                field = field.ToLower();
                if (field.Equals("varchar") || field.Equals("nvarchar") || field.Equals("char") || field.Equals("nchar") || field.Equals("text"))
                {
                    fieldcn = "字符串";
                }
                ...
                else if(field.Equals("binary")|| field.Equals("varbinary"))
                {
                    fieldcn = "二进制";
                }
                ...
                else if (field.Equals("tinyint") || field.Equals("smallint") || field.Equals("tinyint") || field.Equals("int") || field.Equals("bigint"))
                {
                    fieldcn = "数字";
                }
                else if (field.Equals("decimal") || field.Equals("numeric") || field.Equals("smallmoney") || field.Equals("money") || field.Equals("float") || field.Equals("real"))

                {
                    fieldcn = "数字";
                }
                else if (field.Equals("datetime") || field.Equals("datetime2") || field.Equals("smalldatetime") || field.Equals("date") || field.Equals("time") || field.Equals("datetimeoffset") || field.Equals("timestamp"))
                {
                    fieldcn = "日期";
                }
