...
namespace LeaRun.Application.Web.Areas.PublicInfoManage.Controllers
{
    ...
    public class ResourceFileController : MvcControllerBase
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult UploadifyHead( HttpPostedFileBase Filedata)
        {
            try
            {
                Thread.Sleep(500);////延迟500毫秒
                ...
                long filesize = Filedata.ContentLength;
                string FileEextension = Path.GetExtension(Filedata.FileName);                
                string virtualPath = string.Format("~/Resource/StuHeadpic/{0}{1}",   fileGuid, FileEextension);
                string fullFileName = this.Server.MapPath(virtualPath);
                //创建文件夹
                string path = Path.GetDirectoryName(fullFileName);
                ...
                return Success("上传成功。",virtualPath.Substring(1));// 去掉~
            }
            catch (Exception ex)
            {
                return Content(ex.Message);
