...
namespace LeaRun.Application.Web.Areas.FlowManage.Controllers
{
    ...
    public class FlowProcessController : MvcControllerBase
    {
        ...
        {
            var data = wfProcessBll.GetProcessInstanceEntity(keyValue);
            var schemeEntity = wfProcessBll.GetProcessSchemeEntity(data.ProcessSchemeId);
            var queryJons = new { F_ProcessId = keyValue };
            var nodeList = wfProcessBll.ProcessNodesList(queryJons.ToJson());
            var jsonData = new
            {
                processEntity = data,
                processSchemeEntity = schemeEntity,
                nodeList=nodeList
            };
